 ...
 * @package    UIC
 * @subpackage UIC/updater
 */

namespace DKO\UIC\Service;
...
class Plugin_Details {
	...
	public function plugin_links( array $links, string $plugin_file, array $plugin_data ): array {
		if ( false !== strpos( $plugin_data['PluginURI'], 'wc-user-id-sequence-control' ) ) {
			...
			$links[] = sprintf(
				...
				esc_attr( sprintf( __( 'More information about %s' ), 'wc-user-id-sequence-control' ) ),
				__( 'Details', 'wc-user-id-sequence-control' )
			);
		}
